...
def main():
    ...
    if args.display:
        ...
        print s._read_reg(b'\x00\x01')
        print s._read_reg(b'\x00\x02')
        print s._read_reg(b'\x00\x03')
...
class Rego:
    ...
    def _convertHextochr(self,buff):
        text= ""
        for num in range(0,20):
            temp = (ord(buff[0+(num*2)]) << 4) + (ord(buff[1+(num*2)]))
            text += unichr(temp)
        return text
